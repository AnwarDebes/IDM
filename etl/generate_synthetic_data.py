"""
Generate realistic synthetic epidemiological data matching Project Tycho Level 1 schema.

Produces data with:
- Realistic disease-specific temporal patterns (pre/post vaccine decline)
- Seasonal modulation (respiratory diseases peak in winter)
- Population-proportional geographic variation
...
# --- Reference populations by decade (in thousands) ---
# Based on Census data; states not in the reference list are interpolated
REFERENCE_POPS = {
...
# Disease parameters: (base_weekly_rate_per_100k, seasonal_amplitude, season_peak_week, vaccine_year, post_vaccine_decay)
DISEASE_PARAMS = {
...
def get_population(state: str, year: int) -> int:
    """Interpolate population for a state and year."""
...
def compute_expected_cases(disease: str, state: str, year: int, week: int) -> float:
    """Compute expected weekly case count for a disease/state/year/week."""
    params = DISEASE_PARAMS[disease]
    pop = get_population(state, year)

    # Base rate scaled by population
    base = params["base_rate"] * (pop / 100000.0)

    # Seasonal modulation: sinusoidal centered on peak week
    seasonal = 1.0 + params["seasonal_amp"] * math.cos(
        2 * math.pi * (week - params["peak_week"]) / 52.0
    )

    # Vaccine effect: exponential decay after introduction
    vaccine_year = params["vaccine_year"]
    if disease == "SMALLPOX":
        # Smallpox was declining throughout, near zero by 1949
        if year >= 1949:
    ...
    # Decade-based scaling (diseases were generally more prevalent earlier)
    decade_factor = max(0.3, 1.0 - (year - 1930) * 0.003) if year > vaccine_year else 1.0

    expected = base * seasonal * vaccine_effect * decade_factor
    return max(0.0, expected)


def generate_data(output_path: str, start_year: int = 1930, end_year: int = 2010):
    ...
    for year in range(start_year, end_year + 1):
        ...
        for week in range(1, max_week + 1):
            ...
            for state in STATES:
                ...
                for disease in diseases:
                    expected = compute_expected_cases(disease, state, year, week)
        ...
        if year % 10 == 0:
            print(f"  Generated through year {year}: {row_count:,} rows so far")
    ...
    print(f"\nGenerated {row_count:,} rows to {output_path}")
    return row_count


def generate_state_populations(output_path: str):
    """Generate full state_populations.csv for all 50 states by decade."""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["state_code", "decade", "population"])
        for state in STATES:
            for decade in DECADES:
                pop = REFERENCE_POPS[state][decade] * 1000
                writer.writerow([state, decade, pop])
    print(f"Generated state_populations.csv ({len(STATES) * len(DECADES)} rows)")
...
if __name__ == "__main__":
    ...
    print("\nGenerating synthetic Project Tycho dataset...")
    generate_data(os.path.join(data_dir, "raw", "tycho_level1.csv"))
