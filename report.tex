...
Possible extensions include re-measuring MongoDB and Neo4j on the real Project Tycho Level 1 data once a production Docker environment with network access to the Confluent and MongoDB image registries is available, adding time-series database backends (for example TimescaleDB, InfluxDB) for additional comparison, implementing incremental ETL loads rather than full refreshes, and adding more graph-specific analyses that exploit Neo4j's traversal strengths on the city-to-state hierarchy.
